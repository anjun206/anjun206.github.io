...
지금은 비어있는데 필요한 정보나 상태 저장할 멤버들 알아서 추가하라고 한다
...
`vm/anon.c`의 `vm_anon_init`과 `anon_initializer`도 필요에 따라 수정할 수 있다
...
`vm/vm.c`에 `supplemental_page_table_copy`와 `supplemental_page_table_kill`을 구현해야 한다
